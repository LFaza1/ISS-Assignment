...
if ($action == "add_account") {
    ...
    $query = "SELECT * FROM accounts WHERE userid='" . $_SESSION['userid'] . "' AND accountname='" . $name . "'";
    $result = $mysqli->query($query);

    if ($result->num_rows > 0) {
        echo "An account of that name already exists";
    } else {
        $query = "INSERT INTO accounts (accountname, userid) VALUES ('" . $name . "', '" . $_SESSION['userid'] . "')";
        $result = $mysqli->query($query);
    ...
    $query = "SELECT * FROM checks WHERE userid=" . $_SESSION['userid'] . " AND name='" . $name . "'";
    ...
    if ($result->num_rows > 0) {
        ...
        if (count($_FILES) > 0 && array_key_exists('check_file', $_FILES)) {
            ...
            if ($move_result == TRUE) {
                ...
                $insert = "INSERT INTO checks (userid, name, filename) VALUES ('" . $_SESSION['userid'] . "', '" . $name . "', '" . $filename . "')";
