    ...
    class csrfProtector
    {
        ...
        public static function init($length = null, $action = null, $logger = null)
        {
            ...
            if (file_exists($standard_config_location)) {
                ...
                throw new configFileNotFoundException(
                    "OWASP CSRFProtector: configuration file not found for CSRFProtector!"."<br>".$standard_config_location."<br>"."<br>");
            }
            ...
            self::$tokenHeaderKey = 'HTTP_' .strtoupper(self::$config['csrf_token']);
            ...
            if (!isset($_COOKIE[self::$config['csrf_token']])
                || !isset($_SESSION[self::$config['csrf_token']])
                || !is_array($_SESSION[self::$config['csrf_token']])
                || !in_array($_COOKIE[self::$config['csrf_token']],
                $_SESSION[self::$config['csrf_token']])) {
                    self::refreshToken();
            }
        }
        ...
        public static function authorizePost()
        {
            ...
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                ...
                if (!($token && isset($_SESSION[self::$config['csrf_token']])
                ...
                // Currently for same origin only
                if (!(isset($_GET[self::$config['csrf_token']]) 
                    && isset($_SESSION[self::$config['csrf_token']])
                    && (self::isValidToken($_GET[self::$config['csrf_token']])))) {
                    // Action in case of failed validation
                    self::failedValidationAction();
                } else {
                    self::refreshToken();    // Refresh token for successful validation
        ...
        private static function getTokenFromRequest()
        {
            ...
            if (isset($_POST[self::$config['csrf_token']])) {
                return $_POST[self::$config['csrf_token']];
            }

            if (function_exists('getallheaders')) {
                $requestHeaders = getallheaders();
                if (isset($requestHeaders[self::$config['csrf_token']])) {
                    return $requestHeaders[self::$config['csrf_token']];
                }
            }
        ...
        {
            if (!isset($_SESSION[self::$config['csrf_token']])) {
                return false;
            }

            if (!is_array($_SESSION[self::$config['csrf_token']])) {
                return false;
            }

            foreach ($_SESSION[self::$config['csrf_token']] as $key => $value) {
                if ($value == $token) {
                    // Clear all older tokens assuming they have been consumed
                    foreach ($_SESSION[self::$config['csrf_token']] as $_key => $_value) {
                        if ($_value == $token) break;
                        array_shift($_SESSION[self::$config['csrf_token']]);
                    }
        ...
        public static function refreshToken()
        {
            ...
            if (!isset($_SESSION[self::$config['csrf_token']])
                || !is_array($_SESSION[self::$config['csrf_token']]))
                $_SESSION[self::$config['csrf_token']] = array();

            // Set token to session for server side validation
            array_push($_SESSION[self::$config['csrf_token']], $token);
        ...
        public static function ob_handler($buffer, $flags)
        {
            ...
            $hiddenInput = '<input type="hidden" id="' . CSRFP_FIELD_TOKEN_NAME.'" value="'
                            .self::$config['csrf_token'] .'">' .PHP_EOL;
